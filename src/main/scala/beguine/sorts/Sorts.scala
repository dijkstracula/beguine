...
case class Record[T <: Object]() extends IvySort[T] {
  override def inhabitants: Iterator[T] = throw new Exception("TODO?")
}
...
sealed case class Uninterpreted() extends IvySort[Nothing] {
  override def inhabitants: Iterator[Nothing] = throw new Exception("Uninterpreted is an infinite sort")
}
